...
public class RopaBean implements Serializable {
	...
	public List<Articulo> getArticulos() {
		ropa = ModelFacade.obtenerArticulosRopa();

		int inicio = (paginaActual - 1) * articulosPorPagina;
		int fin = Math.min(inicio + articulosPorPagina, ropa.size());

		if (inicio > ropa.size()) {
			return new ArrayList<>();
		}

		return ropa.subList(inicio, fin);
	}
